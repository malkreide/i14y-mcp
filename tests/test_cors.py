...
def client(kind: str, monkeypatch: pytest.MonkeyPatch) -> TestClient:
    """`I14Y_MCP_CORS_ORIGINS` muss gesetzt sein: die Origins sind jetzt
    fail-closed, ein unkonfigurierter Server laesst gar keinen Browser durch."""
    monkeypatch.setenv("I14Y_MCP_CORS_ORIGINS", ORIGIN)
...
async def test_no_tool_declares_an_mcp_param_header() -> None:
    ...
    from i14y_mcp.server import mcp
...
def test_no_configured_origin_means_no_browser_access(
    ...
    """Fail-closed. `allow_origins` was the literal `["*"]` with no way to
    narrow it: every website on the internet could call this server from a
    visitor's browser, and no environment variable existed to say otherwise.

    Unset now means no cross-origin access at all. stdio and non-browser
    clients are unaffected — CORS governs browsers only.
    """
    monkeypatch.delenv("I14Y_MCP_CORS_ORIGINS", raising=False)
...
def test_the_wildcard_is_still_reachable_but_must_be_asked_for(
    ...
    monkeypatch.setenv("I14Y_MCP_CORS_ORIGINS", "*")
    client = TestClient(build_http_app(kind))
    resp = preflight(client, kind, "content-type")
...
def test_configured_origins_parses_a_list(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("I14Y_MCP_CORS_ORIGINS", " https://a.test , https://b.test ")
...
def test_the_transport_check_lets_the_configured_origins_through(
    ...
    """`build_transport_security` carried `{o for o in [] if o != "*"}` — an
    empty comprehension under a comment describing the opposite. There was no
    source configured origins could have come from, so the transport check
    would have rejected exactly the browser clients CORS permits.
    """
    monkeypatch.setenv("I14Y_MCP_CORS_ORIGINS", "https://a.test,*")
