...
def test_the_headers_together(client: TestClient, kind: str) -> None:
    ...
    `allow_origins` is still `["*"]` in this server, so the response echoes `*`
    rather than the request's origin. That wildcard is a separate decision from
    the header list and is left as it stands — changing it would lock out every
    browser client that relies on the current default.
    """
